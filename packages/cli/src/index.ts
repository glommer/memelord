...
if (command === "hook") {
	...
	await withDb(async (db) => {
		const memCount = (
			(await db.prepare("SELECT COUNT(*) as c FROM memories").get()) as any
		).c;
		const taskCount = (
			(await db
				.prepare(
	...
	await withDb(async (db) => {
		const result = await db
			.prepare("DELETE FROM memories WHERE weight < ?")
			.run(threshold);
...
`;
	...
	const selfPkg = findPackageVersionUpwardsFromFile(
		process.argv[1] ?? "",
		"memelord",
	);
	...
	const embedderPkgPath = selfPkg?.rootDir
		...
		: "";
	const embedderPkg = embedderPkgPath
		? readJsonFileIfExists(embedderPkgPath)
		: null;
	const embedderVersion =
		typeof embedderPkg?.version === "string" ? embedderPkg.version : null;
